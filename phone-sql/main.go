...
func insertPhone(db *sql.DB, phone string) (int, error) {
	...
	var id int
	err := db.QueryRow(statement, phone).Scan(&id) // オペラントで id に phoneのid を入れてそのつくられたidを var  id int にいれている（id は自動生成のもの）
	if err != nil {
		return -1, err
	}
	return id, nil
...
func must(err error) { // msutはエラーが出ていないかのみ判定する関数
	if err != nil {
		panic(err)
	}
}
