...
func main() {
	...
	fpdf.ImageOptions("imges/cert.jpg", w/2.0-50.0, pdf.y, 100.0, 0, false, gofpdf.ImageOptions{
		ReadDpi: true,
	}, 0, "")
	...
	sig, err := gofpdf.SVGBasicFileParse("imges/sample.svg")
	if err != nil {
		panic(err)
	}
	...
	// Grid
	// drawGrid(fpdf)
	err = fpdf.OutputFileAndClose("cat.pdf")
	if err != nil {
		panic(err)
	}
}
